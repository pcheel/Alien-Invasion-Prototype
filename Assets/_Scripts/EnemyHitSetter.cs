...
{
    public void Hit(ObjectPool<GameObject> bulletPool, Vector2 position, EnemyHitData hitData);
}
...
public class YellowHitSetter : IEnemyHitSetter
{
    public void Hit(ObjectPool<GameObject> bulletPool, Vector2 position, EnemyHitData hitData)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            bulletGO.GetComponent<Bullet>().StateUpdate(shotDirection, position, hitData._bulletSpeed, hitData._bulletDamage);
        }
    }
}
